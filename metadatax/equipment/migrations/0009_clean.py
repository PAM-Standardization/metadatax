...
from django.db.models import Q, Case, When, F, QuerySet



def _clean_spec(apps, model, distinct, filter, queryset: QuerySet = None):
    ...
    for spec in queryset.distinct(*distinct):
        duplicates = queryset.filter(filter(spec)).filter(~Q(id=spec.id))
        relations = EquipmentModelSpecification.objects.filter(
            specification_type_id=type.id,
            specification_id__in=duplicates.values_list("id", flat=True)
        )
        relations.update(specification_id=spec.id)

        model.objects.filter(id__in=duplicates.values_list("id", flat=True)).delete()
...
def clean_recorder(apps, _):
    ...
    _clean_spec(
        ...
        distinct=[
            "channels_count",
            "storage_slots_count",
            "corrected_storage_maximum_capacity",
            "storage_type",
        ],
        filter=lambda spec: Q(
            channels_count=spec.channels_count,
            storage_slots_count=spec.storage_slots_count,
            corrected_storage_maximum_capacity=spec.corrected_storage_maximum_capacity,
            storage_type=spec.storage_type,
        ),
        queryset=RecorderSpecification.objects.annotate(
            corrected_storage_maximum_capacity=Case(
                When(storage_maximum_capacity="{null,b}", then=None),
                default=F('storage_maximum_capacity'),
