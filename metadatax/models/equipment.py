...
class EquipmentProvider(models.Model):
    ...
    name = models.CharField(
        max_length=255, unique=True, help_text="Name of the manufacturer"
    )
    contact = models.EmailField(
        blank=True, null=True, help_text="Contact email of the manufacturer"
    )
    website = models.URLField(
        blank=True, null=True, help_text="Website of the manufacturer"
    )
...
class RecorderModel(models.Model):
    ...
    provider = models.ForeignKey(
        to=EquipmentProvider,
        related_name="recorder_models",
        on_delete=models.CASCADE,
        help_text="Recorder manufacturer.",
    )
    name = models.CharField(max_length=255, help_text="Name of the recorder model.")
...
class HydrophoneModel(models.Model):
    ...
    name = models.CharField(max_length=255, help_text="Name of the hydrophone model.")
    ...
    def operating_temperature(self) -> str:
        ...
        if (
            self.operating_max_temperature is not None
            and self.operating_min_temperature is not None
        ):
...
class Hydrophone(models.Model):
    ...
    serial_number = models.CharField(
        max_length=255, help_text="Serial number of the hydrophone"
    )
